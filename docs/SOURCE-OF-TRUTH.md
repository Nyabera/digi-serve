...
Approval is limited to the Stage 1 vertical slice and the decisions recorded in version 1.3. Any later change to scope, data model, route classification, permission grants, workflow, status transitions, versioning, navigation, completion, or reopening must update the relevant Markdown documents before implementation.
