...
const OFFICER_APPLICATION_CHECKED_FIELD = "__officerReview:applicationChecked";
const OFFICER_DOCUMENTS_CHECKED_FIELD = "__officerReview:documentsChecked";
const OFFICER_IDENTITY_CHECKED_FIELD = "__officerReview:identityChecked";
const LEGACY_DEPARTMENT_RESULT_FIELD = "__departmentProcessing:financeResult";
const DECISION_RECORD_FIELD = "__supervisorDecision:record";
const DECISION_STATUS_FIELD = "__supervisorDecision:status";
const DECISION_PUBLIC_STATUS_FIELD = "__supervisorDecision:publicStatus";
...
function parseLegacyFinanceResult(
  ...
  try {
    ...
    if (
      ...
      !["CLEAR", "HOLD", "CANNOT_VERIFY"].includes(String(candidate.result)) ||
...
function parseModernFinanceResult(
  ...
  const referralValue = draft["__officerReview:referral"];
  let preferredHandoffId: string | null = null;

  if (typeof referralValue === "string" && referralValue.trim()) {
    try {
      const referral = asRecord(JSON.parse(referralValue));

      if (referral.requestId === requestId && typeof referral.id === "string") {
  ...
  if (!preferredHandoffId) {
    const requestNumber = requestId.match(/REQ-DEMO-(\d+)$/)?.[1];

    if (requestNumber) {
      preferredHandoffId = `HND-DEMO-${requestNumber}`;
    }
  }
  ...
  const handoffPrefix = `__departmentHandoff:${preferredHandoffId}`;
  const resultValue = draft[`${handoffPrefix}:result`];
  const status = draft[`${handoffPrefix}:status`];
  const note = draft[`${handoffPrefix}:resultNote`];
  const completedAt = draft[`${handoffPrefix}:completedAt`];

  if (status !== "COMPLETED" || typeof resultValue !== "string") {
    return null;
  }
  ...
  return {
    ...
    result: resultValue as FinanceResultCode,
    note:
      typeof note === "string"
        ? note
    ...
    completedAt:
      typeof completedAt === "string" ? completedAt : new Date(0).toISOString(),
...
function parseDecision(
  ...
  try {
    ...
    if (
      ...
      typeof candidate.requestId !== "string" ||
      !["APPROVED", "REJECTED", "RETURNED_FOR_CLARIFICATION"].includes(
...
export function SupervisorApprovalWorkspace({
  ...
  const requiredFields = useMemo(
    () =>
      service.form.sections.flatMap((section) =>
  ...
  const financeResult = useMemo(
    () =>
      parseModernFinanceResult(draft, requestId) ??
      parseLegacyFinanceResult(draft[LEGACY_DEPARTMENT_RESULT_FIELD]),
    [draft, requestId],
  );
  ...
  const applicationComplete = requiredFields.every((field) =>
    hasValue(draft[field.key]),
  );

  const requiredDocumentsComplete = requiredDocuments.every((requirement) =>
  ...
  const financeWorkComplete = financeResult !== null;

  const financeClear = financeResult?.result === "CLEAR";
  ...
  ].includes(String(draft[OFFICER_REVIEW_STATUS_FIELD] ?? ""));
  ...
  const rejectionReady =
    applicationComplete && requiredDocumentsComplete && financeResult !== null;
  ...
  const prerequisites = [
    ...
    },
    {
      ...
      detail: String(
        draft[OFFICER_REVIEW_STATUS_FIELD] ?? "No blocking officer state",
      ),
    },
  ] as const;

  function saveValue(fieldKey: string, value: DemoFormValue, at: string) {
    dispatch({
      type: "SET_FORM_VALUE",
  ...
  function saveDecision(
    ...
    const record: RegistrarDecisionRecord = {
      ...
      immutable: decision === "APPROVED" || decision === "REJECTED",
    };

    saveValue(DECISION_RECORD_FIELD, JSON.stringify(record), at);
    saveValue(DECISION_STATUS_FIELD, internalStatus, at);
    saveValue(DECISION_PUBLIC_STATUS_FIELD, publicStatus, at);
    saveValue(OFFICER_REVIEW_STATUS_FIELD, internalStatus, at);
    addActivity(eventName, at);

    setErrorMessage(null);
    setFeedbackMessage(
      `${decision.replaceAll("_", " ")} recorded by the Registrar profile.`,
    );
  }
  ...
  function rejectRequest(event: FormEvent<HTMLFormElement>) {
    ...
    if (!reason) {
      setErrorMessage("Enter an applicant-visible rejection reason.");
      return;
    }
  ...
  function returnForClarification(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const reason = clarificationReason.trim();

    if (!reason) {
      setErrorMessage("Enter the clarification required from Student Records.");
      return;
    }
  ...
  const applicantName = state.applicant.fullName || "Demo applicant";

  const currentStatus =
    existingDecision?.decision ??
    (approvalReady ? "PENDING_APPROVAL" : "BLOCKED");

  const model: SupervisorApprovalDetailModel = {
    requestId,
    serviceName: service.name,
    applicantName,
    applicantInitials: initials(applicantName) || "DA",
    applicantEmail: state.applicant.email || "applicant@example.com",
    applicantPhone: state.applicant.phone || "+254 700 000 000",
    submittedLabel: "14 May 2026, 10:43 AM",
    parentOwnerName: "Student Records",
    registrarDepartmentName: registrarDepartment.name,
    decisionAuthorityName: REGISTRAR_NAME,
    currentStatusLabel: currentStatus.replaceAll("_", " "),
    currentStatusTone:
    ...
    financeResult: financeResult?.result ?? null,
    financeNote:
      financeResult?.note ?? "Complete the Finance work item before approval.",
    financeCompletedBy: financeResult?.completedBy ?? "",
    financeCompletedAtLabel: financeResult
      ? formatTimestamp(financeResult.completedAt)
      : "",
    prerequisites,
    applicationItems: requiredFields.slice(0, 6).map((field) => ({
      id: field.key,
      label: field.label,
      value: formatFieldValue(field, draft[field.key]),
    })),
    documents: service.requiredDocuments.map((requirement) => {
      const available = hasValue(draft[documentFieldKey(requirement.id)]);

      return {
        id: requirement.id,
        name: requirement.name,
        levelLabel: documentLevelLabel(requirement),
        statusLabel: available ? "Available" : "Not selected",
        available,
      };
    }),
    existingDecision: existingDecision
      ? {
          ...
          internalNote: existingDecision.internalNote,
          applicantReason: existingDecision.applicantReason,
          decidedBy: existingDecision.decidedBy,
          decidedAtLabel: formatTimestamp(existingDecision.decidedAt),
        }
      : null,
  };
